...
onAuthStateChanged(auth, async (user) => {
    ...
    const messagesRef = collection(db, "english_chat_form4");
    ...
    onSnapshot(query(messagesRef, where("timestamp", ">", sevenDaysAgo), orderBy("timestamp", "asc")), (snapshot) => {
        ...
        snapshot.forEach(async (docSnapshot) => {
            ...
            if (!messageData.readBy || !messageData.readBy.includes(user.uid)) {
                await updateDoc(doc(db, "english_chat_form4", docSnapshot.id), {
    ...
    document.getElementById("cancelReply").addEventListener("click", () => {
        ...
        messageInput.placeholder = "Type your english question...";
    });
