...
async function loadContent(user, subject) { // 🔥 Pass subject dynamically
    ...
    const subjectCollection = `${subject}_${studentClass}`; // 🔥 Works for any subject (Math, Chemistry, etc.)
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        await loadContent(user, "mathematics"); // 🔥 Call for different subjects
    } else {
        window.location.href = "login.html";
    }
});
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        const subject = "mathematics"; // 🔥 Ensure subject is set dynamically when calling the function
