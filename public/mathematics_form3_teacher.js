...
document.getElementById("uploadBtn").addEventListener("click", async () => {
    ...
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            const subjectCollection = `mathematics_${teacherClass}`; // 🔥 Assign correct collection
...
// ✅ Handle Recording Link Submission
document.getElementById("saveRecordingBtn").addEventListener("click", async () => {
    ...
    if (!recordingLink) {
        document.getElementById("statusMessage").textContent = "🚨 Please paste a valid recording link!";
        return;
    }
    ...
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            const subjectCollection = `mathematics_${teacherClass}`;

            try {
                await setDoc(doc(db, `classes/${subjectCollection}/recordings/${new Date().toISOString()}`), {
                    videoLink: recordingLink,
                    uploadedBy: user.email,
                    timestamp: new Date()
                });

                document.getElementById("statusMessage").textContent = "✅ Recording Link Saved!";
                console.log("✅ Stored under:", `classes/${subjectCollection}/recordings/`);
            } catch (error) {
                document.getElementById("statusMessage").textContent = "🚨 Save failed!";
                console.error("🚨 Firestore error:", error.message);
            }
